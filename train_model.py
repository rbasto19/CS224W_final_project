...
writer = SummaryWriter(log_dir="logs/"+datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
...
for epoch in range(10):
    ...
    writer.add_scalar("Train Loss", batch_loss / len(train_ids), epoch)
    graph_model.eval()
    batch_loss = 0.0
    ...
    writer.add_scalar("Eval Loss", batch_loss / len(train_ids), epoch)